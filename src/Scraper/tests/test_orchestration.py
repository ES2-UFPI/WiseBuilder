...
from SearchEngine.infrastructure.ComponentManagment.SQL_alchemy_repository import SQLAlchemyRepository
from Scraper.infrastructure.VolatileDataManagment.SQL_alchemy_volatile_data import SQLAlchemyVolatileData
...
@pytest.fixture
def get_session():
    engine = create_new_engine('root', '12345678', '127.0.0.1', '3306', 'Wisebuilder_test')
...
def get_volatile_data(get_component):
    ...
    volatile_data = VolatileData(_id=UUIDv5(url.url), url=url, component_id = component.uid, cost=cost, availability=True)
